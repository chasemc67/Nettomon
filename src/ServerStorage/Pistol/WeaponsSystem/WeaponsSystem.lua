...
function WeaponsSystem.setup()
	...
	if IsServer then
		...
		WeaponsSystem.networkFolder = networkFolder
		WeaponsSystem.camera:setEnabled(false) -- changed so cam only activates when weapon is equipped
		game.Players.LocalPlayer.Backpack.Pistol.Equipped:Connect(function()
			WeaponsSystem.camera:setEnabled(true)
		end)

		game.Players.LocalPlayer.Backpack.Pistol.Unequipped:Connect(function()
